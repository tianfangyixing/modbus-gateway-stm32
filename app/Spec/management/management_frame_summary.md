...
| `0x05` | `REQUEST_UPGRADE` | Host → Device | 空 | — |
| `0x85` | `REQUEST_UPGRADE_RESPONSE` | Device → Host | — | `result_code` |
...
| `7` | `INTERNAL_ERROR` | 固件内部不变量、编码、固定容量错误或升级请求寄存器写入/回读校验失败 |
...
```text
0x85 payload = result_code:u16 LE
```

设备将 32 位 RTC 备份寄存器 0 `RTC->BKP0R`（HAL 编号 `RTC_BKP_DR0`，地址 `0x40002850`）写为 `1`，
并回读校验。成功返回 `OK`；非空 payload 返回 `INVALID_REQUEST` 且不写寄存器；备份域写访问不可用或
写入/回读校验失败返回 `INTERNAL_ERROR`。命令不依赖 Configuration、网络或 SNTP 就绪。

`OK` 仅表示升级请求标志已置位，设备继续运行，不自动复位。上位机收到成功响应后，可另行发送已有的
`RESTART` 命令。重复置位允许成功，响应发送失败、USB reset、disconnect 或 session close/open 均不撤销
已写入的标志。

本命令仅对接外部 Bootloader 的寄存器约定：下次复位进入升级模式，以及 USB 初始化成功后将标志清为 `0`，
均由外部 Bootloader 负责，网关应用不执行清零。
