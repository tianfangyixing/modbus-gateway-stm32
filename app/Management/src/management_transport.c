...
#define MANAGEMENT_MESSAGE_REQUEST_UPGRADE UINT8_C(0x05)
#define MANAGEMENT_MESSAGE_REQUEST_UPGRADE_RESPONSE UINT8_C(0x85)
...
static void management_transport_dispatch_request_upgrade(const management_frame_view_t *frame)
{
    uint16_t result_code = 0U;

    if (frame->payload_length != 0U)
    {
        result_code = MANAGEMENT_RESULT_INVALID_REQUEST;
    }
    else if (!boot_request_set())
    {
        result_code = MANAGEMENT_RESULT_INTERNAL_ERROR;
    }

    management_frame_write_u16_le(&tx_buf[MANAGEMENT_FRAME_HEADER_LENGTH], result_code);
    tx_len = 0U;
    if (management_frame_encode(MANAGEMENT_MESSAGE_REQUEST_UPGRADE_RESPONSE, frame->transaction_id,
                                &tx_buf[MANAGEMENT_FRAME_HEADER_LENGTH], 2U, tx_buf, sizeof(tx_buf), &tx_len) !=
        MANAGEMENT_FRAME_OK)
    {
        tx_len = 0U;
    }
...
static void management_transport_dispatch_request(const management_frame_view_t *frame)
{
    ...
    switch (frame->message_type)
    {
        ...
        case MANAGEMENT_MESSAGE_REQUEST_UPGRADE:
            management_transport_dispatch_request_upgrade(frame);
            break;
